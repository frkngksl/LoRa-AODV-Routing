...
void LoRaMac::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        if (!strcmp(addressString, "auto")) {
            ...
            address = DevAddr::generateAutoAddress(); //Mac adresi almak
            // change module parameter from "auto" to concrete address
            par("address").setStringValue(address.str().c_str());
        }
        else
            address.setAddress(addressString);
        registerInterface(); // Interface'e kaydet
...
void LoRaMac::handleUpperPacket(cPacket *msg)
{
    ...
    frame->setLoRaTP(cInfo->getLoRaTP()); //Protokol arası gecisler icin
    ...
    frame->setPacketType(0);
    EV << " Lan geldi mi: " << cInfo->getNodeID();
    frame->setNodeID(cInfo->getNodeID());
    frame->setSequenceNumber(sequenceNumber);
    frame->setReceiverAddress(DevAddr::BROADCAST_ADDRESS); //Frame oluşturduğu kısım direkt BROADCAST ADDRESS YAPIYO
    ++sequenceNumber;
    frame->setLoRaUseHeader(cInfo->getLoRaUseHeader());
    EV << "frame " << frame << " received from higher layer, receiver = " << frame->getReceiverAddress() << endl; // İlk gelen yer
...
{
     //EV << "YAKALADIKKKK\n" ;
    if( (fsm.getState() == RECEIVING_1) || (fsm.getState() == RECEIVING_2)) handleWithFsm(msg); //FSMYE GÖRE BAKIYO
    else{
        //EV << "SİLDİ MUGOOOOO";
        LoRaMacFrame *frame = dynamic_cast<LoRaMacFrame*>(msg);
        EV << "GELEN: " << frame->getNodeID() << "YAKALAYAN: " << this->getParentModule()->getParentModule()->getIndex();
        delete msg;
    }
}
...
void LoRaMac::handleWithFsm(cMessage *msg)
{
    ...
    FSMA_Switch(fsm)
    {
        ...
        FSMA_State(TRANSMIT)
        {
            FSMA_Enter(sendDataFrame(getCurrentTransmission())); // anca burdaysa
