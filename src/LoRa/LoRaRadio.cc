...
{   //dinleme statelerini buraya geçiriyor
    EV_INFO << "Radio mode changed from " << getRadioModeName(previousRadioMode) << " to " << getRadioModeName(newRadioMode) << endl; // 3. gelen mesaj
...
void LoRaRadio::handleMessageWhenUp(cMessage *message)
{
    if (message->isSelfMessage())
        ...
        if (!message->isPacket()) {
            ...
            int index = this->getParentModule()->getParentModule()->getIndex();
            if(index == 1){
                this->setRadioMode(IRadio::RADIO_MODE_RECEIVER);
            }
...
{
    EV << "Bura geldi Handle switchtimer\n";
...
bool LoRaRadio::handleNodeStart(IDoneCallback *doneCallback)
{
    ...
    EV << "Bura geldi Handle node start\n";
...
bool LoRaRadio::handleNodeShutdown(IDoneCallback *doneCallback)
{
    ...
    if (transmissionTimer->isScheduled())
        abortTransmission();
    EV << "Bura geldi Handle node shutdown\n";
    completeRadioModeSwitch(RADIO_MODE_OFF);
...
void LoRaRadio::handleNodeCrash()
{
    ...
    if (transmissionTimer->isScheduled())
        abortTransmission();
    EV << "Bura geldi Handle node crash\n";
    completeRadioModeSwitch(RADIO_MODE_OFF);
...
void LoRaRadio::startTransmission(cPacket *macFrame, IRadioSignal::SignalPart part)
{
    ...
    transmissionTimer->setKind(part); //Returns the radio signal transmission that this radio frame represents
...
// TODO: should be this, but it breaks fingerprints: if (receptionTimer == nullptr && isReceiverMode(radioMode) && arrival->getStartTime(part) == simTime()) {
    if (isReceiverMode(radioMode) && arrival->getStartTime(part) == simTime()) {
        //Buraya geliyor almaya çalıştığında gerci bu gatewayde oluyor
        auto transmission = radioFrame->getTransmission();
    ...
    timer->setKind(part); //teslim alınan data buraya geliyor
...
void LoRaRadio::updateTransceiverState()
{
    ...
    if (receptionState != newRadioReceptionState) {
        //Aha da burasıyla değiştiriyor dinleme yaptıgı kısım
    ...
    if (transmissionState != newRadioTransmissionState) {
        //7. Buraya geldi ya garip
        EV_INFO << "Changing radio transmission state from " << getRadioTransmissionStateName(transmissionState) << " to " << getRadioTransmissionStateName(newRadioTransmissionState) << "." << endl; //4. bura geliyor
        transmissionState = newRadioTransmissionState;
