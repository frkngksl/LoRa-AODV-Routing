...
const ITransmission *LoRaTransmitter::createTransmission(const IRadio *transmitter, const cPacket *macFrame, const simtime_t startTime) const
{
    ...
    const AODVControlPacket *frame2 = check_and_cast<const AODVControlPacket *>(macFrame);
    if(frame2->getPacketType() == 0){
       ...
       int nPreamble = 8;
        ...
        const RREQPacket *frame = check_and_cast<const RREQPacket *> (macFrame);
        int nPreamble = 8;
            simtime_t Tsym = (pow(2, frame->getLoRaSF()))/(frame->getLoRaBW().get()/1000);
            simtime_t Tpreamble = (nPreamble + 4.25) * Tsym / 1000;
        ...
        const RREPPacket *frame = check_and_cast<const RREPPacket *> (macFrame);
                int nPreamble = 8;
                    simtime_t Tsym = (pow(2, frame->getLoRaSF()))/(frame->getLoRaBW().get()/1000);
                    simtime_t Tpreamble = (nPreamble + 4.25) * Tsym / 1000;
