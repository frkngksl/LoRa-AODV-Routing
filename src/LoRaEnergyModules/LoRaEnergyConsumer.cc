        ...
        if(!batteryDown){
            powerConsumption = getPowerConsumption();
                    ...
                    //if (currentSimulationTime != lastEnergyBalanceUpdate) {
                        ...
                        if(energyBalance.get() > powerStorage){
                            batteryDown = true;
                            SimpleLoRaApp* loraApp = dynamic_cast<SimpleLoRaApp *>(getParentModule()->getParentModule()->getParentModule()->getSubmodule("SimpleLoRaApp"));
                            loraApp->dontSend=true;
                        }
                        else{
                            totalEnergyConsumed = (energyBalance.get());
                            EV << "Total Energy Consumed: " << totalEnergyConsumed << "\n";
